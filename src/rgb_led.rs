...
enum TransitionMode {
    Cycle(f32),
    Pulse,
    ShiftHue { start: f32, change: f32 },
    ShiftSaturation { start: f32, change: f32 },
    ShiftValue { start: f32, change: f32 },
}
...
impl Transition {
    fn tick(&mut self, color: &mut Hsv) -> bool {
        ...
        match &self.mode {
            ...
            TransitionMode::ShiftHue { start, change } => {
                Transition::shift_hue(color, progress, start, change)
            }
            TransitionMode::ShiftSaturation { start, change } => {
                Transition::shift_saturation(color, progress, start, change)
            }
            TransitionMode::ShiftValue { start, change } => {
                Transition::shift_value(color, progress, start, change)
            }
        }
    }
...
impl Led {
    pub fn new(
        ledc: LEDC,
        pin_red: impl Peripheral<P = impl OutputPin> + 'static,
        pin_green: impl Peripheral<P = impl OutputPin> + 'static,
        pin_blue: impl Peripheral<P = impl OutputPin> + 'static,
    ) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            channel_r: LedcDriver::new(ledc.channel0, &timer, pin_red)?,
            channel_g: LedcDriver::new(ledc.channel1, &timer, pin_green)?,
            channel_b: LedcDriver::new(ledc.channel2, &timer, pin_blue)?,
            transitions: vec![],
            color: Hsv::new(0., 1., 1.),
            is_cycling: false,
        })
    ...
    pub fn tick(&mut self) -> anyhow::Result<()> {
        self.transitions
            .retain_mut(|transition| !transition.tick(&mut self.color));
